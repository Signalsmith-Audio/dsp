	...
	*/
	...
	struct Hadamard {
		...
		static void inPlace(Data &data) {
			...
			Hadamard<Sample, hSize>::template inPlace<Data, startIndex>(data);
			Hadamard<Sample, hSize>::template inPlace<Data, startIndex + hSize>(data);
	...
	struct Householder {
		...
		constexpr static Sample normFactor() {
			return 1;
		}
	};
	///  @}
